...
function AddPostPage() {
  ...
  const HASH_FORMAT_REGEX =
    /^[a-zA-Z\u0621-\u064A]([a-zA-Z0-9\u0621-\u064A]*_?[a-zA-Z0-9\u0621-\u064A]+)*$/;
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(tempErrors).length === 0) {
      ...
      setLoader(true);
      try {
        const response = await api.post("/public/user_api/add_post.php", {
        ...
        if (response) {
          ...
          toast.success(t(`add_post_page.post_created`));
          setHash("");
          setPost("");
        }
      } catch (error) {
        console.error(error);
        toast.error(t(`add_post_page.create_errore`));
      } finally {
  ...
  return (
    <div className="add_post-page">
      ...
      <span className="page-title">{t(`add_post_page.page_title`)}</span>
      <div className="form-div">
        <span className="title">{t(`add_post_page.form_title`)}</span>
        <p>{t(`add_post_page.form_paragraph`)}</p>
        <form
          action=""
          onSubmit={(e) => {
            handleSubmit(e);
